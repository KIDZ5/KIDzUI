...
local function hashUserId(userId)
	...
	return sha256("kidz_analytics:" .. tostring(userId))
end
...
-- Must match how kidzui.luau discovers these — bare globals, no rawget(_G),
...
--[[
	...
	config = {
		...
		product_name : string?  -- Identifies the product/integration, e.g. "KIDzUI", "MyHub"
		category     : string?  -- Broad type, e.g. "UILibrary", "Product", "Script"
	}
--]]
function Analytics.new(config)
	assert(type(config.url) == "string", "KIDzUIAnalytics: config.url is required")
	assert(type(config.token) == "string", "KIDzUIAnalytics: config.token is required")
...
function Analytics:windowCreated(data)
	...
	local ok2, result2 = pcall(function()
		return _getgenv().KIDZUI_ASSET_ID
	end)
	if ok2 and type(result2) == "number" then
		customAssetId = true
	end
	local ok3, result3 = pcall(function()
		return _getgenv().KIDZUI_SECURE
	end)
