--[[
	...
	KIDzUI Interface Suite
	by KIDz

]]

if debugX then
	warn('กำลังเริ่มต้น KIDzUI')
end
...
local function loadWithTimeout(url: string, timeout: number?): ...any
	...
	local requestThread = task.spawn(function()
		...
		if not fetchSuccess or #fetchResult == 0 then
			if #fetchResult == 0 then
				fetchResult = "ไม่มีข้อมูลตอบกลับ" -- Set the error message
			end
	...
	if not success then
		warn("ไม่สามารถประมวลผลได้ " .. tostring(url) .. ": " .. tostring(result))
	end
...
if _getgenv then
	local ok, result = pcall(function() return _getgenv().DISABLE_KIDZUI_REQUESTS end)
	if ok and result then requestsDisabled = true end
	local ok2, result2 = pcall(function() return _getgenv().KIDZUI_ASSET_ID end)
	if ok2 and type(result2) == "number" then customAssetId = result2 end
	local ok3, result3 = pcall(function() return _getgenv().KIDZUI_SECURE end)
...
local function secureNotify(wType, title, content)
	...
	task.spawn(function()
		while not KIDzUILibrary or not KIDzUILibrary.Notify do task.wait(0.5) end
...
local settingsTable = {
	General = {
		...
		kidzuiOpen = {Type = 'bind', Value = 'LeftControl', Name = 'คีย์ลัดปิด/เปิด UI'},
		-- buildwarnings
		-- kidzuiprompts

	},
	System = {
		usageAnalytics = {Type = 'toggle', Value = true, Name = 'ไม่ระบุตัวตน'},
	}
}
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.kidzuiOpen"] = "J"
...
local prompt = useStudio and require(script.Parent.prompt) or loadWithTimeout('https://raw.githubusercontent.com/KIDZ5/KIDzUI/refs/heads/main/prompt.lua')
local requestFunc = (syn and syn.request) or (fluxus and fluxus.request) or (http and http.request) or http_request or request

-- Validate prompt loaded correctly
if not prompt and not useStudio then
	warn("ไม่สามารถโหลดคลังคำสั่งได้ จึงสลับไปใช้ระบบสำรอง")
	prompt = {
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("KIDzUI | เกิดข้อผิดพลาดทำให้ฟังก์ชันทำงานไม่สำเร็จ: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		if callSafely(isfolder, KIDzUIFolder) then
			if callSafely(isfile, KIDzUIFolder..'/settings'..ConfigurationExtension) then
				file = callSafely(readfile, KIDzUIFolder..'/settings'..ConfigurationExtension)
			end
		end
		...
		if useStudio then
			file = [[
	{"General":{"kidzuiOpen":{"Value":"LeftControl","Type":"bind","Name":"คีย์ลัดปิด/เปิด UI","Element":{"HoldToInteract":false,"Ext":true,"Name":"คีย์ลัดปิด/เปิด UI","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"LeftControl"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"ไม่ระบุตัวตน","Element":{"Ext":true,"Name":"ไม่ระบุตัวตน","Set":null,"CurrentValue":false,"Callback":null}}}}
]]
		end
		...
		-- If no settings saved, apply overridden settings only
			...
			for settingName, settingValue in overriddenSettings do
				...
				assert(#split == 2, "KIDzUI | ระบุชื่อการตั้งค่าที่จะเขียนทับไม่ถูกต้อง: " .. settingName)
	...
	if not success then 
		if writefile then
			warn('KIDzUI พบปัญหาในการเข้าถึงระบบบันทึกการตั้งค่า')
		end
	end
end

if debugX then
	warn('กำลังโหลดข้อมูลการตั้งค่า..')
end

loadSettings()

if debugX then
	warn('โหลดการตั้งค่าสำเร็จ')
end

local ANALYTICS_TOKEN = "ww309dijsfuih328y4982384esdjf08j23urhujnsvsh"

local reporter = nil
if not requestsDisabled and not useStudio then
	local fetchSuccess, fetchResult = pcall((game :: any).HttpGet, game, "https://raw.githubusercontent.com/KIDZ5/KIDzUI/refs/heads/main/reporter.lua")
		...
		if execSuccess and Analytics then
			pcall(function()
				reporter = Analytics.new({
					url          = "https://kidzui-analytics.see2548za.workers.dev/",
...
if promptUser == 1 and prompt and type(prompt.create) == "function" then
	prompt.create(
		'โปรดระมัดระวังขณะเรียกใช้งานสคริปต์',
	    [[โปรดใช้ความระมัดระวังเมื่อรันสคริปต์จากนักพัฒนาที่ไม่รู้จัก สคริปต์นี้ได้ถูกเรียกใช้งานไปแล้ว

<font transparency='0.3'>สคริปต์บางตัวอาจขโมยไอเทมหรือทรัพย์สินในเกมของคุณได้</font>]],
		'ตกลง',
		'',
		function()
...
if debugX then
	warn('กำลังดำเนินการเริ่มต้นระบบต่อไป')
end
...
local KIDzUIAssetId = customAssetId or 121523760801196
local KIDzUI = useStudio and script.Parent:FindFirstChild('KIDzUI') or game:GetObjects("rbxassetid://"..KIDzUIAssetId)[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local kidzuiDestroyed = false -- True when KIDzUILibrary:Destroy() is called

repeat
	if KIDzUI:FindFirstChild('Build') and KIDzUI.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('KIDzUI | เวอร์ชันของระบบไม่ตรงกัน')
		print('KIDzUI อาจเกิดข้อขัดข้อง เนื่องจากเวอร์ชันของ UI ระบบไม่เข้ากัน ('.. ((KIDzUI:FindFirstChild('Build') and KIDzUI.Build.Value) or 'No Build') ..').\n\nKIDzUI รุ่นนี้รองรับการทำงานร่วมกับ UI เวอร์ชัน '..InterfaceBuild..'.')
		warned = true
	end

	local toDestroy
	toDestroy, KIDzUI = KIDzUI, useStudio and script.Parent:FindFirstChild('KIDzUI') or game:GetObjects("rbxassetid://"..KIDzUIAssetId)[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

KIDzUI.Enabled = false

if gethui then
	KIDzUI.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(KIDzUI)
	KIDzUI.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	KIDzUI.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == KIDzUI.Name and Interface ~= KIDzUI then
			Interface.Enabled = false
			Interface.Name = "KIDzUI-Old"
		end
	end
elseif not useStudio then
...
if secureMode and not customAssetId then
	secureNotify("default_asset", "โหมดปลอดภัย", "คุณกำลังใช้งาน Asset ID เริ่มต้นของ KIDzUI โปรดตั้งค่า KIDZUI_ASSET_ID เป็นไฟล์ที่คุณอัปโหลดเองเพื่อหลีกเลี่ยงการถูกตรวจจับ")
end

do
	local AssetPath = KIDzUIFolder.."/Assets"
	local AssetBaseURL = "https://github.com/KIDZ5/KIDzUI/blob/main/assets/"
		...
		local ok, err = pcall(function()
			...
			for id, _ in assetFiles do
				...
				if success then
					...
					warn("KIDzUI | เกิดข้อผิดพลาดในการโหลดไฟล์ Asset ที่คุณกำหนดเอง: "..tostring(id).." - "..tostring(asset))
				end
			end
		end)

		if not ok then
			warn("KIDzUI | เกิดข้อผิดพลาดในการโหลดไฟล์ Asset ที่คุณกำหนดเอง: "..tostring(err))
			secureNotify("asset_load_fail", "KIDzUI", "ไม่สามารถโหลดองค์ประกอบไฟล์ Asset ที่กำหนดเองได้ รูปภาพบนอินเทอร์เฟซอาจแสดงผลไม่ถูกต้อง")
		end
	else
		secureNotify("no_getcustomasset", "KIDzUI", "ตัวรันสคริปต์ (Executor) ของคุณไม่รองรับฟังก์ชัน getcustomasset รูปภาพใน UI บางอย่างอาจแสดงผลไม่ถูกต้อง")
	end


	KIDzUI.Main.Shadow.Image.Image = customAssets[tostring(5587865193)]
	KIDzUI.Main.Topbar.Hide.Image = customAssets[tostring(10137832201)]
	KIDzUI.Main.Topbar.ChangeSize.Image = customAssets[tostring(10137941941)]
	KIDzUI.Main.Topbar.Settings.Image = customAssets[tostring(80503127983237)]
	KIDzUI.Main.Topbar.Icon.Image = customAssets[tostring(78137979054938)]
	KIDzUI.Main.Topbar.Search.Image = customAssets["IconMagnifyingGlass2"]
	KIDzUI.Main.Topbar.Search.ImageRectOffset = Vector2.new(0, 0)
	KIDzUI.Main.Topbar.Search.ImageRectSize = Vector2.new(0, 0)
	KIDzUI.Main.Elements.Template.Toggle.Switch.Shadow.Image = customAssets[tostring(3602733521)]
	KIDzUI.Main.Elements.Template.Slider.Main.Shadow.Image = customAssets[tostring(3602733521)]
	KIDzUI.Main.Elements.Template.Dropdown.Toggle.Image = customAssets["IconChevronTopMedium"]
	KIDzUI.Main.Elements.Template.Dropdown.Toggle.ImageRectOffset = Vector2.new(0, 0)
	KIDzUI.Main.Elements.Template.Dropdown.Toggle.ImageRectSize = Vector2.new(0, 0)
	KIDzUI.Main.Elements.Template.Label.Icon.Image = customAssets[tostring(11745872910)]
	KIDzUI.Main.Elements.Template.ColorPicker.CPBackground.MainCP.Image = customAssets[tostring(11413591840)]
	KIDzUI.Main.Elements.Template.ColorPicker.CPBackground.MainCP.MainPoint.Image = customAssets[tostring(3259050989)]
	KIDzUI.Main.Elements.Template.ColorPicker.ColorSlider.SliderPoint.Image = customAssets[tostring(3259050989)]
	KIDzUI.Main.TabList.Template.Image.Image = customAssets[tostring(4483362458)]
	KIDzUI.Main.Search.Search.Image = customAssets[tostring(18458939117)]
	KIDzUI.Main.Search.Shadow.Image = customAssets[tostring(5587865193)]
	KIDzUI.Notifications.Template.Icon.Image = customAssets[tostring(77891951053543)]
	KIDzUI.Notifications.Template.Shadow.Image = customAssets[tostring(3523728077)]
	KIDzUI.Loading.Banner.Image = customAssets[tostring(111263549366178)]
...
if KIDzUI.AbsoluteSize.X < minSize.X and KIDzUI.AbsoluteSize.Y < minSize.Y then
...
local Main = KIDzUI.Main
local MPrompt = KIDzUI:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = KIDzUI:FindFirstChild('Drag')
...
KIDzUI.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/KIDZ5/KIDzUI/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = KIDzUI.Notifications
local keybindConnections = {} -- For storing keybind connections to disconnect when KIDzUI is destroyed

local SelectedTheme = KIDzUILibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = KIDzUILibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	KIDzUI.Main.BackgroundColor3 = SelectedTheme.Background
	KIDzUI.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	KIDzUI.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	KIDzUI.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	KIDzUI.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	KIDzUI.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	KIDzUI.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		KIDzUI.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		KIDzUI.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(KIDzUI:GetDescendants()) do
...
local function getIcon(name : string): {id: number, imageRectSize: Vector2, imageRectOffset: Vector2}
	if not Icons then
		warn("Lucide Icons: ไม่สามารถใช้ไอคอนได้ เนื่องจากไลบรารีไอคอนยังไม่ได้ถูกโหลด")
		return
	end
	...
	if not r then
		error("Lucide Icons: ไม่พบไอคอนที่ระบุชื่อว่า \"" .. name .. "\"", 2)
	end
	...
	if type(r[1]) ~= "number" or type(rirs) ~= "table" or type(riro) ~= "table" then
		error("Lucide Icons: ข้อผิดพลาดภายในระบบ: รายการข้อมูล Asset ที่สร้างอัตโนมัติไม่ถูกต้อง")
	end
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("KIDzUI | ไม่สามารถใช้งานไอคอน Lucide ได้ เนื่องจากไลบรารีไอคอนยังไม่ได้ถูกโหลด")
	else
		warn("KIDzUI | อาร์กิวเมนต์ (ข้อมูล) ของไอคอน จะต้องเป็นรหัสไอคอน (ตัวเลข) หรือชื่อไอคอน Lucide (ข้อความ) เท่านั้น")
	end
	return assetUri
end
...
local function resolveIcon(icon)
	...
	if secureMode then
		secureNotify("icon_blocked", "โหมดปลอดภัย", "การใช้ไอคอนใน Element โดยอ้างอิงผ่าน Asset ID หรือชื่อ Lucide ถูกบล็อกไว้ โปรดใช้คำสั่ง getcustomasset() สำหรับไอคอนเพื่อไม่ให้ถูกตรวจจับ")
		return "", nil, nil
	end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('KIDzUI เกิดข้อผิดพลาดในการอ่านไฟล์การตั้งค่า กรุณาลองลบไฟล์ดังกล่าวแล้วเปิด KIDzUI ใหม่อีกครั้ง') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(KIDzUILibrary.Flags) do
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(KIDzUILibrary.Flags) do
...
function KIDzUILibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('KIDzUI | ไม่พบการดำเนินการของคุณในการแจ้งเตือนใช่หรือไม่?')
			print("ฟีเจอร์ Notification Actions ปิดใช้งานชั่วคราว")
		end
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			KIDzUILibrary:Notify({Title = "ซ่อนหน้าต่างเมนู", Content = "ซ่อนอินเทอร์เฟซแล้ว คุณสามารถเปิดกลับมาได้โดยแตะ 'Show'", Duration = 7, Image = 4400697855})
		else
			KIDzUILibrary:Notify({Title = "ซ่อนหน้าต่างเมนู", Content = "ซ่อน UI แล้ว คุณสามารถเปิดกลับมาได้โดยแตะ " .. tostring(getSetting("General", "kidzuiOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		callSafely(writefile, KIDzUIFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	if not (writefile and isfile and readfile and isfolder and makefolder) and not useStudio then
		...
		warn('ไม่สามารถสร้างการตั้งค่าได้ เนื่องจากระบบไม่รองรับการบันทึกไฟล์')
		return
	end

	local newTab = window:CreateTab('KIDzUI Settings', 0, true)

	if TabList['KIDzUI Settings'] then
		TabList['KIDzUI Settings'].LayoutOrder = 1000
	end

	if Elements['KIDzUI Settings'] then
		Elements['KIDzUI Settings'].LayoutOrder = 1000
	end
...
function KIDzUILibrary:CreateWindow(Settings)
	if KIDzUI:FindFirstChild('Loading') then
		if getgenv and not getgenv().kidzuiCached then
			KIDzUI.Enabled = true
			KIDzUI.Loading.Visible = true

			task.wait(1.4)
			KIDzUI.Loading.Visible = false
		end
	end

	if getgenv then getgenv().kidzuiCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				KIDzUILibrary:Notify({Title = 'เวอร์ชันของระบบไม่ตรงกัน', Content = 'KIDzUI อาจทำงานผิดปกติ เนื่องจากเวอร์ชัน UI ที่คุณใช้อยู่ไม่เข้ากันกับระบบ ('.. ((KIDzUI:FindFirstChild('Build') and KIDzUI.Build.Value) or 'No Build') ..').\n\nKIDzUI เวอร์ชันนี้ถูกสร้างมาสำหรับบิลด์ UI '..InterfaceBuild..'.\n\nลองออกแล้วเข้าใหม่ จากนั้นรันสคริปต์สองครั้ง', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	if Settings.ToggleUIKeybind then -- Can either be a string or an Enum.KeyCode
		...
		if type(keybind) == "string" then
			...
			end), "ToggleUIKeybind ต้องเป็น KeyCode ที่ถูกต้อง")
			overrideSetting("General", "kidzuiOpen", keybind)
		elseif typeof(keybind) == "EnumItem" then
			assert(keybind.EnumType == Enum.KeyCode, "ToggleUIKeybind ต้องเป็นค่า KeyCode enum")
			overrideSetting("General", "kidzuiOpen", keybind.Name)
		else
			error("ToggleUIKeybind ต้องเป็นข้อความ (String) หรือ KeyCode enum")
		end
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "KIDzUI"
	...
	if Settings.Theme then
		...
		if not success then
			...
			if not success then
				warn('ข้อผิดพลาดร้ายแรง - ไม่พบธีมเริ่มต้น')
				print(result2)
			end
			warn('เกิดปัญหาในการแสดงผลธีม: ไม่พบไฟล์ธีม')
			print(result)
		end
	end
	...
	if not Settings.DisableKIDzUIPrompts then
		task.spawn(function()
			while not kidzuiDestroyed do
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio and not secureMode then
		ensureFolder(KIDzUIFolder.."/Discord Invites")

		if not callSafely(isfile, KIDzUIFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				callSafely(writefile, KIDzUIFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"KIDzUI เปิดใช้งาน RememberJoins สำหรับคำเชิญนี้แล้ว ระบบจะไม่ถามให้คุณกดยืนยันเข้าร่วมซ้ำอีก")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(KIDzUIFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("KIDzUI | "..Key.." ผิดพลาด " ..tostring(Response))
					warn('ดูคู่มือการพัฒนาสำหรับ KIDzUI โดยเฉพาะได้ที่ https://github.com/KIDZ5/KIDzUI/tree/main')
				end
			end
		end
		...
		if callSafely(isfile, KIDzUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, KIDzUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
		...
		if not Passthrough and secureMode then
			warn("KIDzUI | โหมดปลอดภัย: จำเป็นต้องใช้คีย์ที่บันทึกไว้ในระบบล่วงหน้า ไม่สามารถเปิดหน้าต่างสำหรับใส่คีย์ได้ เพื่อหลีกเลี่ยงการโหลดไฟล์ที่เสี่ยงต่อการถูกระบบตรวจจับ")
			KIDzUI.Enabled = false
			return KIDzUILibrary
		end

		if not Passthrough then
			local AttemptsRemaining = Settings.KeySettings.MaxAttempts or 5
			KIDzUI.Enabled = false
			local KeyUI = useStudio and script.Parent:FindFirstChild('Key') or game:GetObjects("rbxassetid://128257222538861")[1]
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, KIDzUIFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						KIDzUILibrary:Notify({Title = "ระบบคีย์", Content = "บันทึกคีย์สำหรับสคริปต์นี้เรียบร้อยแล้ว", Image = 3605522284})
					end
				else
					if AttemptsRemaining == 0 then
						fadeOutKeyUI(KeyMain)
						task.wait(0.45)
						Players.LocalPlayer:Kick("คุณพยายามเกินจำนวนครั้งที่กำหนด")
						game:Shutdown()
					end
	...
	if Settings.KeySystem then
		...
		if kidzuiDestroyed then return end
	end
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called KIDzUILibrary:Destroy()
				if kidzuiDestroyed then
					return
				end
				if not Success then
					...
					Button.Title.Text = "คำสั่งผิดพลาด"
					print("KIDzUI | "..ButtonSettings.Name.." คำสั่งผิดพลาด " ..tostring(Response))
					warn('หากต้องการความช่วยเหลือในการพัฒนา KIDzUI เข้าไปดูคู่มือได้ที่ https://github.com/KIDZ5/KIDzUI/tree/main')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= KIDzUILibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					KIDzUILibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			KIDzUI.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					Input.Title.Text = "คำสั่งผิดพลาด"
					print("KIDzUI | "..InputSettings.Name.." คำสั่งผิดพลาด " ..tostring(Response))
					warn('หากต้องการความช่วยเหลือในการพัฒนา KIDzUI เข้าไปดูคู่มือได้ที่ https://github.com/KIDZ5/KIDzUI/tree/main')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					KIDzUILibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							Dropdown.Title.Text = "คำสั่งผิดพลาด"
							print("KIDzUI | "..DropdownSettings.Name.." คำสั่งผิดพลาด " ..tostring(Response))
							warn('หากต้องการความช่วยเหลือในการพัฒนา KIDzUI เข้าไปดูคู่มือได้ที่ https://github.com/KIDZ5/KIDzUI/tree/main')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					KIDzUILibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							Keybind.Title.Text = "คำสั่งผิดพลาด"
							print("KIDzUI | "..KeybindSettings.Name.." คำสั่งผิดพลาด " ..tostring(Response))
							warn('หากต้องการความช่วยเหลือในการพัฒนา KIDzUI เข้าไปดูคู่มือได้ที่ https://github.com/KIDZ5/KIDzUI/tree/main')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					KIDzUILibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= KIDzUILibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				local Success, Response = pcall(function()
					if debugX then warn('กำลังเรียกใช้งาน Toggle \''..ToggleSettings.Name..'\' (Interact)') end
				...
				if not Success then
					...
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
					Toggle.Title.Text = "คำสั่งผิดพลาด"
					print("KIDzUI | "..ToggleSettings.Name.." คำสั่งผิดพลาด " ..tostring(Response))
					warn('หากต้องการความช่วยเหลือในการพัฒนา KIDzUI เข้าไปดูคู่มือได้ที่ https://github.com/KIDZ5/KIDzUI/tree/main')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			function ToggleSettings:Set(NewToggleValue)
				...
				local Success, Response = pcall(function()
					if debugX then warn('กำลังเรียกใช้งาน Toggle \''..ToggleSettings.Name..'\' (:Set)') end
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						KIDzUILibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								Slider.Title.Text = "คำสั่งผิดพลาด"
								print("KIDzUI | "..SliderSettings.Name.." คำสั่งผิดพลาด " ..tostring(Response))
								warn('หากต้องการความช่วยเหลือในการพัฒนา KIDzUI เข้าไปดูคู่มือได้ที่ https://github.com/KIDZ5/KIDzUI/tree/main')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			function SliderSettings:Set(NewVal)
				...
				if not Success then
					...
					TweenService:Create(Slider.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Transparency = 1}):Play()
					Slider.Title.Text = "คำสั่งผิดพลาด"
					print("KIDzUI | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
					warn('หากต้องการความช่วยเหลือในการพัฒนา KIDzUI เข้าไปดูคู่มือได้ที่ https://github.com/KIDZ5/KIDzUI/tree/main')
					task.wait(0.5)
					Slider.Title.Text = SliderSettings.Name
					TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					KIDzUILibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			KIDzUILibrary:Notify({Title = 'ไม่สามารถเปลี่ยนธีมได้', Content = 'ไม่พบไฟล์ธีมในระบบ', Image = 4400704299})
		else
			KIDzUILibrary:Notify({Title = 'เปลี่ยนธีมแล้ว', Content = 'เปลี่ยนธีมเป็นสำเร็จ '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('KIDzUI พบปัญหาในการสร้างการตั้งค่า') end
...
function KIDzUILibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function KIDzUILibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function KIDzUILibrary:Destroy()
	kidzuiDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['KIDzUI Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "kidzuiOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function KIDzUILibrary:LoadConfiguration()
	local config

	if debugX then
		warn('กำลังโหลดการตั้งค่า')
	end
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				KIDzUILibrary:Notify({Title = "การตั้งค่า KIDzUI", Content = "ระบบไม่สามารถเปิดใช้งานฟังก์ชันบันทึกการตั้งค่าได้ เนื่องจากซอฟต์แวร์ที่ท่านใช้งานไม่รองรับการทำงานร่วมกับระบบจัดการไฟล์ (Filesystem)", Image = 4384402990})
			end
		end)
...
if useStudio then
	...
	--local Window = KIDzUILibrary:CreateWindow({
	--	Name = "KIDzUI Example Window",
	--	LoadingTitle = "KIDzUI Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	--	LoadingSubtitle = "by KIDz",
	--	ConfigurationSaving = {
	--		Enabled = true,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using KIDzUI may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like KIDzUI to get the key from
	...
	----KIDzUILibrary:Notify({Title = "KIDzUI Interface", Content = "Welcome to KIDzUI. These - are the brand new notification design for KIDzUI, with custom sizing and KIDzUI calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(KIDzUILibrary.Theme) do
...
task.delay(4, function()
	KIDzUILibrary.LoadConfiguration()
